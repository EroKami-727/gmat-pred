...
const ARCHS = [
  {
    ...
    specs: [
      ...
      ['Pos. enc', 'Sinusoidal + CLS prepend'],
...
const PIPELINE = [
  ['# 1. Generate dataset (2 × 5K missions)', 'dim'],
  ...
  ['', 'dim'],
  ['# 2. Train transformer (recommended: 40% early exit)', 'dim'],
  ['python3 -m src.ml.train \\', 'info'],
  ['  --data data/merged/missions.parquet \\', 'dim'],
  ['  --model transformer --epochs 30 --early-exit 0.4 \\', 'dim'],
  ['  --output-dir models/transformer_production', 'dim'],
  ['', 'dim'],
  ['# 3. Ablation sweep (all fractions, feeds dashboard)', 'dim'],
  ['python3 -m src.ml.ablation \\', 'info'],
  ['  --data data/merged/missions.parquet --model transformer --epochs 30', 'dim'],
  ['', 'dim'],
  ['# 4. Start API + frontend', 'dim'],
  ['uvicorn src.api.main:app --port 8000 --reload', 'info'],
  ['cd frontend && npm run dev', 'info'],
]
